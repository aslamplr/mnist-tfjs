...
export function imageDataToGrayscale(imgData: ImageData) {
  var grayscaleImg: number[][] = [];
...
export function getBoundingRectangle(img: number[][], threshold: number) {
  var rows = img.length;
...
export function centerImage(img: number[][]) {
  var meanX = 0;
  var meanY = 0;
  var rows = img.length;
  var columns = img[0].length;
